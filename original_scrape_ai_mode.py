...
LOW_DATA_MODE = os.getenv('LOW_DATA_MODE', '1') != '0'
...
def enable_low_data_mode(driver: uc.Chrome) -> None:
    """Enable low-data mode by blocking images and other heavy resources."""
    try:
        driver.execute_cdp_cmd("Network.enable", {})
        driver.execute_cdp_cmd("Network.setBlockedURLs", {
            "urls": [
                "*.png", "*.jpg", "*.jpeg", "*.gif", "*.webp", "*.svg",
                "data:image/*", "*.woff2", "*.woff", "*.ttf", "*favicon*"
            ]
        })
        print("✅ Low-data mode enabled - blocking images and fonts")
    except Exception as e:
        print(f"Warning: Could not enable low-data mode: {e}")

def init_driver_session() -> uc.Chrome:
    """Initialize a persistent Chrome driver session at Google AI Mode start page."""
    driver = create_driver()
    if LOW_DATA_MODE:
        enable_low_data_mode(driver)
    go_to_google_start(driver)
    return driver
